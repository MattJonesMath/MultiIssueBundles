...
if __name__ == '__main__':
    ...
    for i in range(iters):
        ...
        params = [[flip, utilities, odd_partitions] for flip in flips]
        result = pool.map(sim, params)
        big_results.append(result)
            
    mins_by_sample = [min(l1) for l1 in big_results]
    print(min(mins_by_sample))
...
#########
plt.subplots()
plt.hist(mins_by_sample,bins = [-1, -10/11, -8/11, -6/11, -4/11, -2/11, 0, 2/11, 4/11, 6/11, 8/11, 10/11, 12/13, 1])
...
plt.subplots()
plt.hist(mins_by_sample,bins = [-10/11, -8/11, -6/11, -4/11, -2/11, 0])
plt.xlabel('Issue Score of Subbundle Scheme')
plt.ylabel('Frequency')
plt.tight_layout()
plt.savefig('FigSI4.png', dpi=300)