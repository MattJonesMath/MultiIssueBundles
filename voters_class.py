...
class Voters:
    def __init__(self, n, m, util_method, util_import = [], utils = [-2,-1,1,2], util_probs = [0.25,0.25,0.25,0.25], spatial_dim=2, spatial_skew=0.5, trade_net='complete', trade_thresh=1, bundle_split='random', bundle_num=2):
        self.n = n
        self.m = m
        ...
        if self.util_method == 'iid':
            ...
            for issue in self.issue_positions:
                ...
                for i in range(self.n):
                    ...
                    if self.dist(v_pos, no_pos) > self.dist(v_pos, yes_pos):
                        temp = 2*yes_pos - no_pos
                        if self.dist(v_pos, temp)>self.dist(v_pos,no_pos):
                            self.utilities[i].append(self.utils[2])
                        else:
                            self.utilities[i].append(self.utils[3])
                    else:
                        temp = 2*no_pos - yes_pos
                        if self.dist(v_pos, temp)>self.dist(v_pos, yes_pos):
                            self.utilities[i].append(self.utils[1])
                        else:
                            self.utilities[i].append(self.utils[0])
